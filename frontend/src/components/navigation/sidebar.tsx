...
import { Home, Compass, Library, Heart, User, Settings } from "lucide-react";
  ...
  return (
    <aside className="fixed left-0 top-0 h-full w-64 bg-[#080808] border-r border-zinc-900 flex flex-col z-[100] font-sans">
      ...
      <nav className="flex-1 px-4 mt-4">
        ...
        <div className="mt-16 px-4">
          ...
          <div className="flex flex-col gap-4">
            {["Design_Systems", "Backend_Core", "Visual_Assets"].map(
              (tag, idx) => (
                <button
                  key={tag}
      ...
      <div className="p-4 border-t border-zinc-900 bg-[#080808]">
        <Link
          href="/admin"
          className="flex items-center justify-between group cursor-pointer p-4 rounded-xl hover:bg-zinc-900/50 transition-all duration-300"
        >
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-full border border-zinc-800 flex items-center justify-center bg-zinc-900/50 group-hover:border-white transition-colors overflow-hidden">
              <User
                size={14}
            ...
            <div className="flex flex-col">
              <span className="text-[10px] text-zinc-200 font-bold tracking-wider uppercase group-hover:text-white transition-colors">
                User_Admin
              </span>
              <span className="text-[8px] text-zinc-600 tracking-tighter uppercase font-mono group-hover:text-zinc-400 transition-colors">
                Stockholm_HQ
              </span>
            </div>
          </div>
          <Settings
            size={12}
            className="text-zinc-700 group-hover:text-white group-hover:rotate-45 transition-all duration-500"
          />
        </Link>
      </div>
    </aside>
